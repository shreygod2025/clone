...
curl -L -o oll_backup.tar.gz \
  ...
  https://inquiry-edit-bug.preview.emergentagent.com/api/admin/db-backup/download
```
...
curl -X POST \
  -H "Authorization: Bearer YOUR_TOKEN" \
  https://inquiry-edit-bug.preview.emergentagent.com/api/admin/db-backup/create
```
