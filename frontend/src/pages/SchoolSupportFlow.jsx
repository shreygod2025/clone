...
import { ArrowLeft, Package, UserX, Monitor, CreditCard, ExternalLink, Camera, Check } from 'lucide-react';
...
const SchoolSupportFlow = ({ onBack }) => {
  const [step, setStep] = useState('category'); // category, subcategory, form, success
  ...
  const paymentSubCategories = [
    { id: 'invoice', label: 'Want Invoice', action: 'form' },
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.school_name || !formData.class_division || !formData.phone) {
      toast.error('Please fill all required fields');
      return;
    }
    setSubmitting(true);
    try {
      await axios.post(`${API}/support/school-query`, {
        category,
  ...
  const renderCategorySelection = () => (
    <div className="space-y-3">
      <h2 className="text-xl font-bold text-[#1E3A5F] mb-4">Questions related to:</h2>
      <p className="text-slate-500 text-sm mb-6">Ongoing classes in school</p>
      {categories.map(cat => (
        <div
          key={cat.id}
  ...
  const renderSubCategory = () => {
    ...
    if (category === 'teacher') {
      return (
        <div className="space-y-3">
          <h2 className="text-xl font-bold text-[#1E3A5F] mb-4">Teacher Related</h2>
          <div
            className="selection-card p-4 cursor-pointer"
            onClick={() => {
              setSubCategory('complaint');
              setStep('form');
            }}
            data-testid="teacher-complaint"
          >
            <h3 className="font-semibold text-[#1E3A5F]">Complaint Against Teacher</h3>
            <p className="text-sm text-slate-500">Share your concerns or feedback</p>
          </div>
        </div>
      );
    }
  ...
  const renderForm = () => {
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {(category === 'teacher' || (category === 'kit' && subSubCategory)) && (
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">
              {category === 'teacher' ? 'Details of your concern *' : 'Additional details (optional)'}
            </label>
            <Textarea
              placeholder={category === 'teacher' ? 'Please describe the issue...' : 'Describe the issue...'}
        ...
        {subCategory === 'invoice' && (
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl">
            <p className="text-sm text-blue-700">
              We will send the invoice to your WhatsApp/Email.
            </p>
          </div>
        )}
...
export default SchoolSupportFlow;
