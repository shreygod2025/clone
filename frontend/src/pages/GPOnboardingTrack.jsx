...
import { 
  CheckCircle2, User, FileText, GraduationCap, 
  Phone, Mail, Clock, ArrowLeft, ExternalLink, Handshake, MapPin
} from 'lucide-react';
...
const ONBOARDING_STEPS = [
  { key: 'personal_info', label: 'Personal Information', icon: User, description: 'Complete your personal and bank details' },
  { key: 'contract_signing', label: 'Contract Signing', icon: FileText, description: 'Review and sign the partnership contract' },
  { key: 'training', label: 'Training', icon: GraduationCap, description: 'Complete the partner training program' },
];

const GPOnboardingTrack = () => {
  ...
  const fetchOnboarding = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${API}/gp-onboarding/track/${token}`);
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex items-center justify-center gap-3 mb-2">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-white">Partner Onboarding</h1>
          </div>
          <p className="text-white/80">Track your onboarding journey as an OLL Growth Partner</p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 p-6 text-white">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-white/80 capitalize">{onboarding.interest_type || 'Growth Partner'}</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-4 text-sm">
                ...
                {onboarding.city && (
                  <span className="flex items-center gap-1">
                    <MapPin className="w-4 h-4" /> {onboarding.city}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="p-6 border-b">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-lg font-bold text-orange-600">{completedSteps}/3 Complete</span>
              </div>
              ...
              {onboarding.status === 'active' && (
                <div className="mt-4 p-3 bg-green-50 rounded-lg text-green-700 text-sm">
                  ...
                  Congratulations! You are now an active OLL Growth Partner.
                </div>
              )}
              {onboarding.status === 'discontinued' && (
                <div className="mt-4 p-3 bg-red-50 rounded-lg text-red-700 text-sm">
                  This partnership has been discontinued.
                </div>
              )}
            </div>
            ...
            <div className="px-6 py-4 bg-slate-50 text-center text-sm text-slate-500">
              ...
              <a href="mailto:partners@oll.co" className="text-orange-600 hover:underline">partners@oll.co</a>
            </div>
          </div>
        </div>
      </div>
...
export default GPOnboardingTrack;
