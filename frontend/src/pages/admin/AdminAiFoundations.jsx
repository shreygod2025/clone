...
import {
  ...
  TrendingUp, Users, IndianRupee, Award, ExternalLink, Save
} from 'lucide-react';
...
const AdminAiFoundations = () => {
  ...
  return (
    <AdminLayout title="AI Foundations">
      <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5" data-testid="admin-ai-foundations">
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-[#1E3A5F] flex items-center gap-2">
              <Sparkles className="w-7 h-7 text-blue-600" /> AI Foundations CRM
            </h1>
            <p className="text-sm text-slate-500 mt-1">Every lead, every booking, all on one page.</p>
          </div>
                ...
                <tbody>
                  {filtered.map(b => (
                    <tr key={b.id}
                      ...
                      data-testid={`booking-row-${b.booking_ref}`}>
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-[11px] text-slate-500">Grade {b.student_grade}</div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="text-xs text-slate-700 flex items-center gap-1.5"><Phone className="w-3 h-3" /> {b.parent_phone}</div>
                        <div className="text-[11px] text-slate-500 flex items-center gap-1.5"><Mail className="w-3 h-3" /> {b.parent_email}</div>
                      </td>
                      <td className="px-4 py-3"><StatusBadge status={b.crm_status} payment={b.payment_status} /></td>
                      <td className="px-4 py-3 text-right text-xs text-slate-500">{fmtDate(b.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/50 flex justify-end" onClick={(e) => { if (e.target === e.currentTarget) setSelected(null); }}>
          <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl" data-testid="booking-detail-drawer">
            <div className="sticky top-0 bg-white border-b border-slate-100 p-4 flex items-center justify-between">
              <div>
                <div className="text-[10px] uppercase tracking-widest text-blue-600 font-bold">Booking</div>
...
export default AdminAiFoundations;
