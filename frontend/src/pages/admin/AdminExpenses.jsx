        ...
        {loading ? (
          ...
          <div className="divide-y">
            {summary.schools?.map((school) => (
              <div key={school.school_id} className="hover:bg-slate-50/50">
                <div 
                  className="p-4 flex items-center justify-between cursor-pointer"
                  onClick={() => setExpandedSchool(expandedSchool === school.school_id ? null : school.school_id)}
                >
                  <div className="flex items-center gap-3">
                    <Building2 className="w-5 h-5 text-slate-400" />
                    <div>
                      <p className="font-medium text-slate-800">{school.school_name}</p>
                      <p className="text-sm text-slate-500">
                        {Object.keys(school.by_category || {}).length} expense categories
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <p className="text-lg font-semibold text-[#1E3A5F]">
                      {formatCurrency(school.total_expenses)}
                    </p>
                    {expandedSchool === school.school_id ? 
                ...
                {expandedSchool === school.school_id && (
                  <div className="px-4 pb-4 pt-0">
                    <div className="bg-slate-50 rounded-lg p-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                      {Object.entries(school.by_category || {}).map(([catId, data]) => {
                        const cat = categories.find(c => c.id === catId);
                        return (
                          <div key={catId} className="bg-white rounded-lg p-3 border">
                            <span className={`text-xs px-2 py-1 rounded-full ${getCategoryColor(catId)}`}>
                              {cat?.name || catId}
                            </span>
                    ...
                    <div className="mt-3 flex gap-2">
                      <Button 
                        ...
                        onClick={() => {
                          resetForm();
                          setExpenseForm(prev => ({...prev, school_id: school.school_id}));
                          setShowAddModal(true);
                        }}
                      >
