...
const AiFoundationsSuccessPage = () => {
  ...
  useEffect(() => {
    ...
    // Trial flow — no Cashfree verification needed
    if (isTrial) {
      (async () => {
        try {
          ...
          setStatus('trial'); // still show success even if verify fails
        }
      })();
      return;
    }
        ...
        <div className="bg-white border-2 border-slate-100 rounded-3xl p-8 lg:p-12 shadow-2xl shadow-blue-900/10 text-center">
          ...
          {status === 'trial' && (
            <>
              ...
              <span className="inline-flex items-center gap-1.5 text-xs font-bold tracking-widest text-emerald-700 uppercase">
                <Sparkles className="w-3.5 h-3.5" /> Free Trial Confirmed
              </span>
              <h1 className="text-3xl lg:text-4xl font-black text-[#0F1E33] mt-2">
                You{`'`}re in{booking?.student_name ? `, ${booking.student_name.split(' ')[0]}` : ''}! 🎉
              </h1>
              <p className="text-slate-600 mt-3 max-w-xl mx-auto">
                {booking?.booking_ref && <>Booking <span className="font-mono font-bold text-[#1E3A5F]">{booking.booking_ref}</span> · </>}
                Session 1 is on us. We{`'`}ll WhatsApp the joining link shortly.
              </p>
              <div className="mt-8 bg-blue-50 border border-blue-100 rounded-2xl p-5 text-left text-sm text-slate-700 space-y-2">
                ...
                <ol className="list-decimal list-inside space-y-1 text-slate-600">
                  <li>You{`'`}ll get a WhatsApp message with the join link before Session 1.</li>
                  <li>Attend Session 1 — completely free, no card needed.</li>
                  <li>If your child loves it, pay ₹1,999 to continue sessions 2 – 10.</li>
                </ol>
              </div>
              <button
                onClick={() => navigate('/')}
