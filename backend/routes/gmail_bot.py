...
async def gmail_auth_url(user: dict = Depends(get_current_user)):
    ...
    # Persist state + PKCE code_verifier so callback can verify both. 10-min TTL.
    # The verifier is generated by google-auth-oauthlib's Flow on `authorization_url()` and
    # must be replayed on `fetch_token()` to satisfy Google's PKCE check.
