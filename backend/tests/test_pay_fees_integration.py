...
class TestPayFeesIntegration:
    ...
    def test_student_inquiry_endpoint(self):
        """Test GET /api/students endpoint to verify student data"""
        # Get student inquiries
        response = requests.get(f"{BASE_URL}/api/students")
        
        # Check if auth is required
        if response.status_code == 401:
            print("✓ Student endpoint requires authentication (expected)")
            return
        
        assert response.status_code == 200
        data = response.json()
        
        # Look for test student
        test_student = None
        for student in data:
            if student.get("phone") == "9876543210":
                test_student = student
                break
        
        if test_student:
            assert "pending_payment" in test_student, "Student should have pending_payment field"
            print(f"✓ Test student found with pending payment")
        else:
            print("⚠ Test student not found in list (may require auth)")
